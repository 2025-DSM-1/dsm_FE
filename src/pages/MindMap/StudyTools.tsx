...
const data = [
  ...
  },
  {
    ...
    answer:
      "노동자가 산재로 사망했을 때 기업의 경영책임자를\n처벌하기 위해 제정된 법이다.",
    reason:
      "중대재해처벌법은 산업현장에서 노동자가 산재로 사망했을 때 등의 중대재해가 발생했을 때 경영책임자를 처벌하기 위해 규정한 법입니다.",
  },
  {
    ...
    answer: "주 40시간",
    reason:
      "근로기준법상 법정 근로시간은 주 40시간이며, 연장근로 포함 시 최대 52시간까지 가능합니다.",
  },
  {
    ...
    reason:
      "법률안 공포는 대통령이 법률 확정을 공식적으로 국민에게 알리는 절차입니다.",
  },
  {
    ...
    answer: "국민투표로 확정",
    reason:
      "일반 법안은 국민투표를 거치지 않고, 국회 의결 후 대통령 공포로 확정됩니다. 국민투표는 헌법 개정 등 특정 사안에만 해당됩니다.",
  },
]
    ...
    <Container>
      <HeaderSection>
        <CountWrap>
          ...
          <ProblemCount>{currentIndex + 1} / {data.length}</ProblemCount>
        </CountWrap>
      ...
      <QuizSection>
        <ProblemOptionWrapper>
          <ProblemWrapper>
            <QuestionIcon src={Question} alt="Question" />
            <ProblemText>{currentQuestion.problem}</ProblemText>
          </ProblemWrapper>
        ...
        {showAnswer && (
          <AnswerBox isCorrect={isCorrect}>
            <AnswerTitleWrapper>
              {isCorrect ? (
                <Check color="#1C8543" size={20} />
              ) : (
                <X color="#D63636" size={20} />
              )}
              <AnswerTitle>{isCorrect ? "맞았습니다" : "틀렸습니다"}</AnswerTitle>
            </AnswerTitleWrapper>
            <AnswerReason>{currentQuestion.reason}</AnswerReason>
          </AnswerBox>
        )}
...
export const ProblemCount = styled.p`
  font-size: 14px;
  font-weight: 400;
`
